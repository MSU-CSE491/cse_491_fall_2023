...
protected:
  enum ActionType { REMAIN_STILL = 0, MOVE_UP, MOVE_DOWN, MOVE_LEFT, MOVE_RIGHT, MOVE_ARBITRARY };
...
public:
  ...
  [[nodiscard]] bool IsTraversable(const AgentBase & /*agent*/,
                                   cse491::GridPosition pos) const override
  {
    // return !GetCellTypes().at(main_grid.At(pos)).HasProperty(CellType::CELL_WALL);
    //  ^ This doesn't work because we're not assigning any properties to the cell types, so a
    //  band-aid solution is to use name
