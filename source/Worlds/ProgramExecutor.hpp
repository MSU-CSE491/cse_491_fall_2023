...
public:
  ...
  ProgramExecutor()
  {
    ...
    registerFunction("for", [this](ProgramExecutor& pe) {
      ...
      if ((start < end && step > 0 && value <= end) || (start >= end && step < 0 && value >= end)) {
  ...
  ProgramExecutor(cse491::WorldBase& world) : ProgramExecutor()
  {
    ...
    registerFunction("setProperty", [this, &world](ProgramExecutor& pe) {
      ...
      Entity& agent =
          world.HasAgent(id) ? static_cast<Entity&>(world.GetAgent(id)) : world.GetItem(id);
    ...
    registerFunction("addInventoryItem", [this, &world](ProgramExecutor& pe) {
      ...
      Entity& entity = world.HasAgent(owner_id) ? static_cast<Entity&>(world.GetAgent(owner_id))
                                                : world.GetItem(owner_id);
  ...
  void pushStack(Value value) { stack.push(value); }
      ...
      switch (unit.type) {
        ...
        case Unit::Type::operation:
          ...
          if (unit.value == "=") {
            ...
            std::transform(identifier_values.begin(), identifier_values.end(),
            ...
            if (type == "__FOR_BLOCK") {
              ...
              if ((start < end && step > 0 && value + step <= end) ||
                ...
                // continue
                variables.insert_or_assign(static_cast<std::string>(as<Identifier>(info.at(1))),
                                           value + step);
                // jump back to the beginning to check the condiiton
                //									index =
                //static_cast<int>(as<double>(call_stack.top().at(5)));
              } else {
                // end loop
                call_stack.pop();
                skipBlock(1);
