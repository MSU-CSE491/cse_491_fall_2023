...
public:
  ManualWorld()
  {
    ...
    portal_id_a =
        AddCellType("portal_a", "Portal that teleports player to another a-portal spot.", '}');
    portal_id_b =
        AddCellType("portal_b", "Portal that teleports player to another b-portal spot.", '{');
    portal_id_c =
        AddCellType("portal_c", "Portal that teleports player to another c-portal spot.", '(');
    portal_id_d =
        AddCellType("portal_d", "Portal that teleports player to another d-portal spot.", ')');
    ...
    for (auto& [id, agent_ptr] : agent_map) {
      ...
      if (agent_ptr->GetName() == "Interface") {
        std::map<std::string, std::tuple<char, double>> move_set = {
            ...
            {"Special", std::make_tuple('d', 1.5)},
            {"Run", std::make_tuple('d', 0.0)},
            {"Heal", std::make_tuple('h', 0.25)}};
        // agent_ptr->SetProperty<std::map<std::string, std::tuple<char, double>>>("MoveSet",
        // move_set);
  ...
  void AddMove(cse491::AgentBase& agent, std::string& move, char stat, double modification)
  {
    if (!agent.HasProperty("MoveSet")) {
      ...
      std::map<std::string, std::tuple<char, double>> move_set =
          agent.GetProperty<std::map<std::string, std::tuple<char, double>>>("MoveSet");
  ...
  int OtherAction(cse491::AgentBase& other_agent, cse491::AgentBase& agent)
  {
    int other_damage = 0;
    std::map<std::string, std::tuple<char, double>> move_set =
        other_agent.GetProperty<std::map<std::string, std::tuple<char, double>>>("MoveSet");
    ...
    if (stat_char == 's') {
      if (stat_modification < 0) {
        ...
        int new_strength =
            static_cast<int>(agent_strength - abs(stat_modification) * agent_strength);
        agent.SetProperty<int>("Strength", new_strength);
      } else {
        int other_agent_strength = other_agent.GetProperty<int>("Strength");
        int new_strength_other =
            static_cast<int>(other_agent_strength + abs(stat_modification) * other_agent_strength);
  ...
  void DoBattle(cse491::AgentBase& other_agent, cse491::AgentBase& agent, char attack_type)
  {
    ...
    switch (attack_type) {
      case 'a':
      case 'A':
        damage = agent.GetProperty<int>("Strength");
        break;
      case 's':
      case 'S':
        damage = static_cast<int>(agent.GetProperty<int>("Strength") * 1.5);
        break;
      case 'r':
      case 'R':
        won = false;
      ...
      case 'B':
        agent.SetProperty<int>("Strength",
                               static_cast<int>(1.5 * agent.GetProperty<int>("Strength")));
        break;
      case 'h':
      case 'H':
        HealAction(agent);
    ...
    // Other Agent Move Choice, will modify strength of player agent or NPC agent based on the move
    // selected
    ...
    DataCollection::DataManager::GetInstance().GetAgentInteractionCollector().RecordInteraction(
        other_agent_name);

    if (!won) {
      if (run) {
        agent.Notify(
            "You ran away, this means you don't gain health or strength and any battle damage "
            "stays!\n");
      }
  ...
  void DoActionAttemptItemPickup(cse491::AgentBase& agent, const cse491::GridPosition& new_position)
  {
    for (const auto& [id, item_ptr] : item_map) {
      if (item_ptr->GetPosition() == new_position && item_ptr->GetOwnerID() == 0) {
        ...
        if (item_ptr->GetName() == "Stick" || item_ptr->GetName() == "Sword") {
          uses_property = "Strength";
        }
        if (item_ptr->GetName() == "Boat" || item_ptr->GetName() == "Axe") {
          uses_property = "Uses";
        }
        if (item_ptr->GetName() == "Health Potion") {
          uses_property = "Healing";
        }
        ...
        if (uses_property == "Strength") {
          if (agent.HasProperty(uses_property)) {
            agent.SetProperty(uses_property, item_ptr->GetProperty<int>(uses_property) +
                                                 agent.GetProperty<int>(uses_property));
          } else {
            agent.SetProperty(uses_property, item_ptr->GetProperty<int>(uses_property));
          }
        }

        DataCollection::DataManager::GetInstance().GetItemUseCollector().IncrementItemUsage(
            item_ptr->GetName());
  ...
  /// If the agent can interact with the tree, prompts the user if they want to use one of their
  /// chops
  ...
  void DoActionTestNewPositionTree(cse491::AgentBase& agent,
                                   const cse491::GridPosition& new_position)
  {
    ...
    if (item_id != SIZE_MAX) {
      agent.Notify("You have used your Axe to chop down this tree. You have " +
                   std::to_string(item_map[item_id]->GetProperty<int>("Uses") - 1) +
                   " uses remaining");
  ...
  int DoAction(cse491::AgentBase& agent, size_t action_id) override
  {
    ...
    if (main_grid.At(new_position) == water_id) {
      ...
      if (action_id != USE_BOAT && main_grid.At(agent.GetPosition()) != water_id) {
        return false;
      }
    }
  ...
  [[nodiscard]] bool IsTraversable(const cse491::AgentBase& /*agent*/,
