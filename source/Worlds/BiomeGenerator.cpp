 ...
 */
BiomeGenerator::BiomeGenerator(BiomeType biome, size_t width, size_t height, unsigned int seed)
    : biome(biome), width(width), height(height), seed(seed)
{
...
void BiomeGenerator::setWorld(WorldBase *world) { worldPtr = world; }
 ...
 */
void BiomeGenerator::placeDoorTile(const size_t &doorTile) { grid.At(2, 2) = doorTile; }
 ...
 */
void BiomeGenerator::placeSpecialTiles(const size_t &genericTile, const size_t &specialTile,
                                       double percentage)
{
...
void BiomeGenerator::saveToFile(const std::string &filename) const
{
  ...
  types.push_back(
      CellType{"tar", "Slow tile that makes you take two steps to get through it", 'O'});
  types.push_back(
      CellType{"key", "item that can be picked up to unlock door and escape maze", 'K'});
  types.push_back(CellType{
      "door", "Door that can be walked through only with possession of key to leave maze", 'D'});
...
void BiomeGenerator::placeTrees()
{
  ...
  for (unsigned int y = 1; y < height - 1; ++y) {
    for (unsigned int x = 1; x < width - 1; ++x) {
      ...
      if (grid.At(x, y) == grass_id && grid.At(x - 1, y) == grass_id &&
          grid.At(x + 1, y) == grass_id && grid.At(x, y - 1) == grass_id &&
          grid.At(x, y + 1) == grass_id && grid.At(x - 1, y - 1) == grass_id &&
          grid.At(x + 1, y - 1) == grass_id && grid.At(x - 1, y + 1) == grass_id &&
          grid.At(x + 1, y + 1) == grass_id) {
...
void BiomeGenerator::oceanHandler()
{
  for (size_t y = 1; y < height - 1; ++y) {
    for (size_t x = 1; x < width - 1; ++x) {
      if (grid.At(x, y) == water_id && worldPtr->GetRandom(100) < 15) {
        for (int i = -1; i <= 1; ++i) {
          for (int j = -1; j <= 1; ++j) {
