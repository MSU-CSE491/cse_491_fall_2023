...
protected:
  ...
  void ConfigAgent(AgentBase &agent) override
  {
    ...
    agent.SetProperty("key_property",
                      0.0);  ///< If key is set to 0, agent does not have possession of key. If it
                             ///< is 1, agent has possession of key and can exit through door
    agent.SetProperty(
        "tar_property",
        5.0);  ///< If it is set to 5.0, agent is free to move. if it is set to 6.0, agent is stuck
  }
   ...
   */
  void CreateGrid(BiomeType biome, unsigned int width, unsigned int height, unsigned int seed,
                  const string &file)
  {
...
public:
  explicit GenerativeWorld(BiomeType biome, unsigned int width, unsigned int height,
                           unsigned int seed)
      : WorldBase(seed)
  {
  ...
  void AddArmory()
  {
    ...
    while (!counter) {
      ...
      int random_y =
          (int)GetRandom((double)main_grid.GetHeight() / 2, (double)main_grid.GetHeight() - 1);
    ...
    while (!counter) {
      ...
      int random_x =
          (int)GetRandom((double)main_grid.GetWidth() / 2, (double)main_grid.GetWidth() - 1);
  ...
  bool IsTraversable(const AgentBase & /*agent*/, cse491::GridPosition pos) const override
  {
    ...
    return !(tileType == wall_id || tileType == spike_id || tileType == tar_id ||
             tileType == armory_id || tileType == teleporter_id);
  }
