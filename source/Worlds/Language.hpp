...
struct string : pegtl::seq<pegtl::one<'"'>, pegtl::star<pegtl::not_one<'"'>>, pegtl::one<'"'>> {};
...
struct identifier_list
    : pegtl::sor<pegtl::seq<identifier, pegtl::one<','>, identifier_list>, identifier> {};
...
struct op_prio_comp
    : pegtl::sor<TAO_PEGTL_STRING("=="), TAO_PEGTL_STRING("!="), TAO_PEGTL_STRING("<="),
                 TAO_PEGTL_STRING(">="), pegtl::one<'<'>, pegtl::one<'>'>> {};
...
// "id(arg1,arg2,...)" or "id()"
struct function
    : pegtl::seq<identifier, pegtl::one<'('>, pegtl::opt<expression_list>, pegtl::one<')'>> {};
...
struct element : pegtl::sor<function, identifier, number, string,
                            pegtl::seq<pegtl::one<'('>, expression, pegtl::one<')'>>> {};

// Matches one multiplication expression or a single element
// "a*b" or "c"
struct mul_a : pegtl::sor<pegtl::seq<element, op_prio_mul, element>, element> {};

struct mul;
// Recursively matches multiplication operations
// Ex. "1*4", "a*b*c"
struct mul : pegtl::sor<pegtl::seq<mul_a, op_prio_mul, mul>, mul_a> {};

// Matches one addition expression or a multiplication subexpression
// "a+b" or "a+b*c", with "b*c" as the subexpression
struct add_a : pegtl::sor<pegtl::seq<mul, op_prio_add, mul>, mul> {};

// Matches entire addition expression
struct add;
struct add : pegtl::sor<pegtl::seq<add_a, op_prio_add, add>, add_a> {};

struct comp_a : pegtl::sor<pegtl::seq<add, op_prio_comp, add>, add> {};

// Matches entire comparison expression
struct comp;
struct comp : pegtl::sor<pegtl::seq<comp_a, op_prio_comp, comp>, comp_a> {};

// Match an expression intended to evaluate to a single value.
struct expression : pegtl::sor<comp> {};

struct expression_list;
// A list of expressions. Can contain one or more expressions (not zero!)
// "123,345+456,func(3)+6*8"
struct expression_list
    : pegtl::sor<pegtl::seq<expression, pegtl::one<','>, expression_list>, expression> {};

// Assignment expression
// "var=value", or "var,vars=val1,val2", etc.
struct assignment : pegtl::seq<identifier_list, pegtl::one<'='>, expression_list> {};
...
// ex. { /*code*/ }
struct code_block
    : pegtl::seq<pegtl::one<'{'>, pegtl::eol, statement_list, pegtl::one<'}'>, pegtl::eol> {};

struct statement : pegtl::sor<pegtl::seq<function, pegtl::opt<code_block>, pegtl::opt<pegtl::eol>>,
                              pegtl::seq<assignment, pegtl::opt<pegtl::eol>>, comment, pegtl::eol> {
};

struct statement_list;
// function()\nfunction()...
struct statement_list : pegtl::sor<pegtl::seq<statement, statement_list>, statement> {};

struct program : pegtl::seq<statement_list, pegtl::eolf> {};
...
using selector = tao::pegtl::parse_tree::selector<
    Rule,
    tao::pegtl::parse_tree::store_content::on<number, string, function, identifier, identifier_list,
                                              expression, expression_list, statement,
                                              statement_list, code_block, program, assignment,
                                              op_prio_add, op_prio_mul, op_prio_comp>,
...
std::vector<Unit> parse_to_code(std::string program)
{
  ...
  std::function<void(const std::unique_ptr<pegtl::parse_tree::node>&)> traverse =
    ...
    if (type == "worldlang::number") {
      ...
      if (node->children.size() > 1) traverse(node->children[1]);

      //				out.push_back(Unit{Unit::Type::number,
      ...
      // expression
      //				out.push_back(Unit{Unit::Type::number,
      //std::to_string(identifier_count)});
      out.push_back(Unit{Unit::Type::operation, "="});
    } else if (type == "worldlang::mul_a" || type == "worldlang::add_a" ||
    ...
    } else if (type == "worldlang::expression_list" || type == "worldlang::identifier_list") {
      log << "Traversing " << type << "\n";
      ...
      if (node->children.size()) {
        ...
        if (first->type == "worldlang::function" && first->children.at(0)->string() != "if" &&
            first->children.at(0)->string() != "for" && node->children.size() > 1 &&
      ...
      log << "Type: " << type;
      if (node->has_content()) log << " Content: " << node->string();
      log << std::endl;
      // visit all children
      for (const auto& child : node->children) {
        traverse(child);
