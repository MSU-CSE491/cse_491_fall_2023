 ...
 * @brief A mechanism of identifying a grid cell, as well as a position within
 *the cell.
...
/// The position is stored as floating-point values (to allow for smooth motion
...
class GridPosition {
 ...
 public:
  ...
  [[nodiscard]] GridPosition Above(double dist = 1.0) const {
    return GetOffset(0.0, -dist);
  }

  /// Return a grid position below this one (by default, directly below)
  [[nodiscard]] GridPosition Below(double dist = 1.0) const {
    return GetOffset(0.0, dist);
  }

  /// Return a grid position to the left of this one (by default, directly left)
  [[nodiscard]] GridPosition ToLeft(double dist = 1.0) const {
    return GetOffset(-dist, 0.0);
  }

  /// Return a grid position to the right of this one (by default, directly
  /// right)
  [[nodiscard]] GridPosition ToRight(double dist = 1.0) const {
    return GetOffset(dist, 0.0);
  }
