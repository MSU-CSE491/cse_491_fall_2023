...
protected:
  ...
  WorldGrid
      &main_grid; ///< Main grid for this world; shortcut to `grids["main"]`
  type_options_t
      type_options; ///< Vector of types of cells in grids for this world.
  ...
  std::uniform_real_distribution<>
      uni_dist; ///< Uniform distribution of doubles, 0 to 1
...
public:
  ...
  WorldBase(unsigned int seed = 0)
    ...
    // The first cell type (ID 0) should be reserved for errors or empty spots
    // in a grid.
    AddCellType("Unknown",
  ...
  [[nodiscard]] size_t GetItemID(const std::string &name) {
    for (size_t i = 0; i < item_set.size(); ++i) {
      if (item_set[i] && item_set[i]->GetName() == name)
        return i;
    }
    return npos;
  }
  ...
  [[nodiscard]] size_t GetAgentID(const std::string &name) {
    for (size_t i = 0; i < agent_set.size(); ++i) {
      if (agent_set[i] && agent_set[i]->GetName() == name)
        return i;
    }
    return npos;
  }
  ...
  virtual const WorldGrid &GetGrid(const std::string &name) const {
    return grids.at(name);
  }
  ...
  WorldBase &RemoveAgent(std::string agent_name = "None") {
    assert(agent_name !=
           "Interface"); // We are not allowed to remove interfaces.
  ...
  virtual void RunAgents() {
    for (size_t i = 0; i < agent_set.size(); ++i) {
      if (!agent_set[i])
        continue; // Skip over deleted agents.
      size_t action_id = agent_set[i]->SelectAction(main_grid, type_options,
                                                    item_set, agent_set);
    ...
    if (agent_receiver != nullptr) {
      for (const auto &agent_ptr : agent_set) {
        agent_receiver->StoreData(agent_ptr->GetName(),
                                  agent_ptr->GetPosition(),
                                  agent_ptr->GetActionResult());
      }
    }
  }
  ...
  /// @brief Determine if this tile can be walked on, defaults to every tile is
  /// walkable
  /// @author @mdkdoc15
  /// @param pos The grid position we are checking
  /// @return If an agent should be allowed on this square
  virtual bool IsTraversable(const AgentBase & /*agent*/,
