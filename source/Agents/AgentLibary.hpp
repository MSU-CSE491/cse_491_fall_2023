...
inline std::vector<cse491::GridPosition> GetShortestPath(const cse491::GridPosition &start,
                                                         ...
                                                         const cse491::AgentBase &agent)
{
  ...
  if (!(world.GetGrid().IsValid(start) && world.GetGrid().IsValid(end))) return path;
  ...
  std::priority_queue<std::shared_ptr<walle::Node>, std::vector<std::shared_ptr<walle::Node>>,
                      walle::CompareNodes>
      openList;
 ...
 */
inline std::vector<cse491::GridPosition> StrToOffsets(std::string_view commands)
{
 ...
 */
template <typename T>
T &DownCastAgent(cse491::Entity &entity)
  requires(Agent_Type<T>)
{
