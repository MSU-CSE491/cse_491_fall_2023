...
public:
  ...
  {
    possibleInstructionsList =
        EncodeActions(action_map, sensorsNamesList, operationsList, actionsList);
  ...
  void GenerateRandomActionList()
  {
    ...
    for (int i = 0; i < LISTSIZE; i++) {
      instructionsList.push_back(
          std::make_tuple(possibleInstructionsList[dist(gen)], dist2(gen), dist2(gen)));
    }
  }

  /// @brief Encodes the actions from an agent's action map into a vector of string, representing
  /// action names.
                   ...
                   [[maybe_unused]] const cse491::agent_map_t &agent_set) override
  {
    ...
    while (i < LISTSIZE * 2 && action.empty()) {
      if (std::find(actionsList.begin(), actionsList.end(), std::get<0>(instruction)) !=
          actionsList.end()) {
        ...
        action = std::get<0>(instruction);
                           ...
                           std::get<0>(instruction)) != sensorsNamesList.end()) {
  ...
  void SerializeGP(tinyxml2::XMLDocument &doc, tinyxml2::XMLElement *parentElem,
                   double fitness = -1) override
  {
    ...
    if (fitness != -1) listElem->SetAttribute("fitness", fitness);
  ...
  void Import(const std::string &encodedLists) override
  {
    ...
    while (comma_pos != std::string::npos) {
      ...
      decodedInstructionsList.push_back(
          std::make_tuple(decodedInstruction.substr(0, first_period_pos),
                          std::stoi(decodedInstruction.substr(
                              first_period_pos + 1, second_period_pos - first_period_pos + 1)),
                          std::stoi(decodedInstruction.substr(second_period_pos + 1))));
    ...
    std::string unseparated_instruction_list = encodedLists.substr(
        first_semicolon_pos + 1, second_semicolon_pos - first_semicolon_pos + 1);
    ...
    while (period_pos != std::string::npos) {
      decodedPossInstructionsList.push_back(
          unseparated_instruction_list.substr(start_pos, period_pos));
