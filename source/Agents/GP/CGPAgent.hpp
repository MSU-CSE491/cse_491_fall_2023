...
public:
  ...
  bool Initialize() override
  {
    ...
    if (genotype.GetNumFunctionalNodes() == 0) {
      genotype = CGPGenotype(
          {INPUT_SIZE, action_map.size(), NUM_LAYERS, NUM_NODES_PER_LAYER, LAYERS_BACK});
    }
                   ...
                   double fitness = -1) override
  {
    ...
    if (fitness != -1) genotypeElem->SetAttribute("fitness", fitness);
  ...
  double GetComplexity() const
  {
    ...
    // Just needed some function such that connection_complexity + node_complexity grows as the
    // number of nodes grows, this function makes the increase more gradual.
