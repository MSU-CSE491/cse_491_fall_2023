...
/// @brief A namespace for base64 encoding and decoding. Does not convert to and from base64 in the
/// typical way. Only guarantees that x == b64_inv(b64(x)), aside from doubles which have problems
/// with precision, so x ~= b64_inv(b64(x)).
namespace base64 {
...
static std::string ULLToB64(size_t ull)
{
  ...
  // Or x + 1 <= b^n -> log_b(x + 1) <= n -> ceil(log_b(x + 1)) = n; But this can cause an overflow
  // when adding 1 to the max value of size_t
...
/// @brief Converts a double to a base64 string. Assumes that the stoull(to_string(value)) is
/// possible. Only guarantees that x ~= b64_inv(b64(x)) due to precision errors. Empirically
/// accurate up to 3 decimal places, e.g. round(x, 3) = round(b64_inv(b64(x)), 3).
...
static std::string DoubleToB64(double value)
{
  ...
  // Location of the decimal from the right end of the string, so that leading 0s that are dropped
  // can be recovered
...
struct CGPParameters {
  ...
  CGPParameters(size_t num_inputs, size_t num_outputs, size_t num_layers,
                size_t num_nodes_per_layer, size_t layers_back)
      : num_inputs(num_inputs),
        num_outputs(num_outputs),
        num_layers(num_layers),
        num_nodes_per_layer(num_nodes_per_layer),
        layers_back(layers_back)
  {
  }
...
private:
  ...
  std::string EncodeGenotype() const
  {
    ...
    for (const CGPNodeGene &node : nodes) {
      // Input Connections
      genotype += base64::B2ToB64(
          std::string(node.input_connections.cbegin(), node.input_connections.cend()));
  ...
  /// @brief Decodes the genotype string and configures the node genes. Node gene vector should be
  ...
  void DecodeGenotype(const std::string &genotype)
  {
    ...
    while (node_gene_end != std::string::npos) {
      ...
      // If there were leading bits that were 0 when converted to base 64, they were dropped. Add
      // them back.
      std::string input_connections_str =
          std::string(input_connections.cbegin(), input_connections.cend());
      assert(input_connections.size() >=
             input_connections_b2.size());  // Invalid genotype if this fails
      input_connections_b2 =
          std::string(input_connections.size() - input_connections_b2.size(), '0') +
          input_connections_b2;
...
public:
  ...
  CGPGenotype() = default;
  /// @brief Constructor for the cartesian graph genotype. Initializes the genotype with the given
  /// parameters and leaves everything default (nodes will be unconnected).
  ...
  bool HasInputConnections() const
  {
    for (auto it = begin(); it != end(); ++it) {
      if (std::any_of(it->input_connections.begin(), it->input_connections.end(),
                      [](char c) { return c != '0'; }))
        return true;
    }
    return false;
  ...
  void InitGenotype()
  {
    ...
    for (size_t i = 1; i <= params.num_layers + 1; ++i) {
      size_t layer_size =
          i == params.num_layers + 1 ? params.num_outputs : params.num_nodes_per_layer;
  ...
  /// @param mutation The function to use for mutating the output. The function will receive the
  /// node gene as a parameter.
  /// @return This genotype.
  CGPGenotype &Mutate(double mutation_rate, GPAgentBase &agent,
                      std::function<void(CGPNodeGene &)> mutation)
  {
    assert(mutation_rate >= 0.0 && mutation_rate <= 1.0);
    for (CGPNodeGene &node : nodes)
      if (agent.GetRandom() < mutation_rate) mutation(node);
    return *this;
  }

  /// @brief Mutates the input connections of the genotype.
  /// @param mutation_rate The probability of mutating a connection. For a given connection, if it
  /// is chosen to be mutated, there is a 50% chance it will stay the same.
  ...
  /// @brief Mutates the genotype by changing the function of each node with a given probability
  /// between 0 and 1.
  ...
  {
    Mutate(mutation_rate, agent, [num_functions, &agent](CGPNodeGene &node) {
      node.function_idx = agent.GetRandomULL(num_functions);
    });
    return *this;
  }

  /// @brief Mutates the genotype, changing the default output of nodes with probability between 0
  /// and 1.
  /// @param mutation_rate Value between 0 and 1 representing the probability of mutating each
  /// value.
                             ...
                             bool additive = true)
  {
    Mutate(mutation_rate, agent, [mean, std, &agent, additive](CGPNodeGene &node) {
      ...
      if (additive) {
        ...
        // Wrap random double in stod(to_string(.)) to reliably export and import genotype from
        // string.
  ...
  CGPGenotype &MutateHeader(double mutation_rate, GPAgentBase &agent)
  {
    ...
    // Can mutate number of inputs and outputs to adapt to changing state and action spaces, but not
    // doing it for now
    ...
    if (agent.GetRandom() < mutation_rate) {
      ...
      for (size_t i = 1; i <= params.num_layers + 1; ++i) {
        ...
        for (size_t j = 0; j < layer_size; ++j) {
          ...
          auto &curr_connections =
              nodes[(i - 1) * params.num_nodes_per_layer + j].input_connections;
          ...
          if (num_needed > 0) {
            ...
            curr_connections.insert(curr_connections.cbegin(), input_connections.cbegin(),
                                    input_connections.cend());
          }
        }
      }
    }
    ...
    if (agent.GetRandom() < mutation_rate) {
      ...
      for (size_t i = 1; i <= params.num_layers + 1; ++i) {
        ...
        new_nodes.insert(new_nodes.cend(), nodes.cbegin() + layer_start,
                         nodes.cbegin() + layer_end);
        ...
        for (size_t j = 0; j < layer_size; ++j) {
          // Add an empty connection at the end of each layer of connections in the valid layers
          // back
          assert(new_layer_start + j < new_nodes.size());
          auto &connections = new_nodes[new_layer_start + j].input_connections;
          // Only iterate over the valid layers back that are middle layers, not including the input
          // layer
      ...
      for (size_t i = 1; i < params.num_layers + 1; ++i) {
        ...
        for (size_t j = 0; j < layer_size; ++j) {
          ...
          [[maybe_unused]] size_t num_connections = valid_layers_back * params.num_nodes_per_layer;
          if (i <= params.layers_back) {
  ...
  /// @param num_functions The number of functions available to the nodes.
  /// @return This genotype.
  CGPGenotype &MutateDefault(double mutation_rate, GPAgentBase &agent,
