...
private:
  ...
  tinyxml2::XMLElement *rootTopLastGenerations =
      lastGenerationsTopAgentsDoc.NewElement("GPLoopLastNGeneration");
  tinyxml2::XMLElement *rootAllOfLastGeneration =
      allOfLastGeneration.NewElement("GPLoopAllOfLastGeneration");
   ...
   * Default Grid
   */
  const std::vector<cse491::GridPosition> STARTPOSITIONS = {
      cse491::GridPosition(0, 0), cse491::GridPosition(22, 5), cse491::GridPosition(22, 1),
      cse491::GridPosition(0, 8), cse491::GridPosition(22, 8)};
  //        const std::vector<cse491::GridPosition> STARTPOSITIONS = {cse491::GridPosition(0,0),
  //        cse491::GridPosition(22,5) }; const std::vector<cse491::GridPosition> STARTPOSITIONS =
  //        {cse491::GridPosition(22,5) }; const std::vector<cse491::GridPosition> STARTPOSITIONS =
  //        {cse491::GridPosition(0,0)};

  /**
   * Group 8 Grid
   */
  //        const std::vector<cse491::GridPosition> STARTPOSITIONS = {cse491::GridPosition(0,2),
  //        cse491::GridPosition(49,2) , cse491::GridPosition(49,19) , cse491::GridPosition(4,19),
  //        cse491::GridPosition(28,10)};

  /**
   * Default Grid    2
   */
  //        const std::vector<cse491::GridPosition> STARTPOSITIONS = {cse491::GridPosition(0,0),
  //        cse491::GridPosition(50,0) , cse491::GridPosition(0,28) , cse491::GridPosition(50,28)};

  /// ArenaIDX, AgentIDX, EndPosition
  std::vector<std::vector<std::vector<cse491::GridPosition>>> endPositions =
      std::vector<std::vector<std::vector<cse491::GridPosition>>>();
  std::vector<std::vector<std::vector<double>>> independentAgentFitness =
...
public:
  ...
  void Initialize(size_t numArenas = 5, size_t NumAgentsForArena = 100)
  {
    ...
    for (size_t i = 0; i < numArenas; ++i) {
      ...
      for (size_t j = 0; j < NumAgentsForArena; ++j) {
        ...
        cowboys::GPAgentBase &addedAgent = static_cast<cowboys::GPAgentBase &>(
            environments[i]->template AddAgent<AgentType>("Agent " + std::to_string(j)));
  ...
  std::filesystem::path FullLoadGrabLatestGeneration()
  {
    ...
    for (const auto &entry : std::filesystem::directory_iterator(normalizedAbsolutePath)) {
      if (entry.path().extension() == lastGenerationsFilenameExtension &&
          entry.path().filename().string().find(lastGenerationsPrefix) != std::string::npos) {
  ...
  double AStarFitnessFunction(const cse491::GridPosition &startpos,
                              const cse491::GridPosition &endpos, int arena, int a)
  {
    double fitness = 0;

    // Euclidean distance
    cse491::GridPosition currentPosition = endpos;
    double distance = static_cast<double>(Get_A_StarDistance(startpos, currentPosition, arena, a));
  ...
  /**
   * @brief: runs the Genetic Programming training loop for a number of generations to evolve the
   * agents
   *
   ...
   * @param maxThreads
   */
  void Run(size_t numGenerations, size_t numberOfTurns = 100, size_t maxThreads = 0,
           bool saveData = false)
  {
    ...
    for (; generation < numGenerations; ++generation) {
      ...
      std::cout << "Generation " << generation << " took " << generationDuration.count() / 1000000.0
                << " seconds" << std::endl;
      analyzer.saveToFile();
    }
  ...
  void SaveDataCheckPoint(const SaveDataParams &params)
  {
    ...
    SerializeAgents(params.generation, rootAllOfLastGeneration, allOfLastGeneration,
                    totalNumberOfAgents);
  ...
  size_t Get_A_StarDistance(const cse491::GridPosition &startpos,
                            const cse491::GridPosition &endpos, int arenaIDX, int agentIDX)
  {
  ...
  int AgentsAnalysisComputationsAndPrint(int generation, double deltaForMaxFitness = 0.1)
  {
    ...
    std::cout << "Best agent: AGENT[" << bestAgent.first << "," << bestAgent.second << "] "
              << std::endl;
    ...
    auto calculateDistance = [](const cse491::GridPosition &startPosition,
                                const cse491::GridPosition &currentPosition) {
    ...
    std::cout << std::left << std::setw(columnWidth) << "Start" << std::setw(columnWidth) << "Final"
              << std::setw(columnWidth) << "Distance"
              << "A* Distance\n";
    for (size_t i = 0; i < STARTPOSITIONS.size(); ++i) {
      std::cout << std::setw(columnWidth) << FormatPosition(STARTPOSITIONS[i])
                << std::setw(columnWidth)
                << FormatPosition(endPositions[bestAgent.first][bestAgent.second][i]);

      // Calculate and print Euclidean distance
      double distance =
          calculateDistance(STARTPOSITIONS[i], endPositions[bestAgent.first][bestAgent.second][i]);
      std::cout << std::fixed << std::setprecision(2) << std::setw(12) << distance;

      // Calculate and print A* distance
      size_t astarDistance =
          Get_A_StarDistance(startPosition[i], endPosition[i], bestAgent.first, bestAgent.second);
      std::cout << std::setw(12) << astarDistance;

      std::cout << std::endl;
    }

    std::cout << "with best agent weighted score of "
              << TEMPAgentFitness[bestAgent.first][bestAgent.second] << std::endl;
    std::cout << std::endl;
   ...
   * @param topN
   */
  void SerializeAgents(int generation, tinyxml2::XMLElement *rootElement,
                       tinyxml2::XMLDocument &paramDocument, size_t topN = 5)
  {
    std::string tagName = "generation_" + std::to_string(generation);
    ...
    for (size_t i = 0; i < std::min(sortedAgents.size(), topN); ++i) {
      ...
      agents[arenaIDX][agentIDX]->SerializeGP(paramDocument, generationTag,
  ...
  void GpLoopMutateHelper()
  {
    ...
    //          std::min(static_cast<int>(std::thread::hardware_concurrency()), global_max_threads);
    ...
    for (int i = 0; i < num_threads; ++i) {
      ...
      threads.push_back(std::thread(
          [this, start, end] { this->MutateAgents(start, end, sortedAgents, agents, 0.05); }));
    }
  ...
  void RunArena(size_t arena, size_t numberOfTurns)
  {
    for (size_t startPos_idx = 0; startPos_idx < STARTPOSITIONS.size(); ++startPos_idx) {
      ...
      for (size_t a = 0; a < agents[arena].size(); ++a) {
        //              double tempscore = SimpleFitnessFunction(*agents[arena][a],
        //              STARTPOSITIONS[startPos_idx]);
        double tempscore = AStarFitnessFunction(STARTPOSITIONS[startPos_idx],
                                                agents[arena][a]->GetPosition(), arena, a);
