...
public:
  ...
  {
    if (layers.size() == 0) return actions.at(0);

    // Set inputs of input layer
    size_t i = 0;
    for (auto &node : layers[0]) {
      double input = 0;
      if (i < inputs.size()) input = inputs.at(i);
  ...
  /// @brief Add a layer to the graph. Purely organizational, but important for CGP for determining
  /// the "layers back" parameter.
                                ...
                                const std::unordered_map<std::string, double> &extra_agent_state)
{
  ...
  double above_state =
      grid.IsValid(current_position.Above()) ? grid.At(current_position.Above()) : 0.;
  double below_state =
      grid.IsValid(current_position.Below()) ? grid.At(current_position.Below()) : 0.;
  double left_state =
      grid.IsValid(current_position.ToLeft()) ? grid.At(current_position.ToLeft()) : 0.;
  double right_state =
      grid.IsValid(current_position.ToRight()) ? grid.At(current_position.ToRight()) : 0.;
  ...
  inputs.insert(inputs.end(), {prev_action, starting_x, starting_y, distance_from_start,
