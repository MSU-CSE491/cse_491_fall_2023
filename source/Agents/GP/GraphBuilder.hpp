...
/// @brief A class for building graphs. Graphs are a generic representation, so this class is used
/// to build the specific format of a Cartesian Graph, and also preset graphs.
class GraphBuilder {
public:
                                        ...
                                        const cse491::AgentBase *agent = nullptr)
  {
    ...
    for (; nodes_it != functional_nodes.end() && genes_it != genotype.cend();
         ++nodes_it, ++genes_it) {
  ...
  std::unique_ptr<Graph> VerticalPacer()
  {
    ...
    input_layer.insert(input_layer.end(), {prev_action, current_state, above_state, below_state,
