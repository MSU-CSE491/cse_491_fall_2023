...
public:
  ...
  {
    if (cached_output_valid) return cached_output;
  ...
  template <size_t N>
  std::optional<std::vector<double>> GetInputValues(const std::array<size_t, N> &indices) const
  {
    size_t max_index = *std::max_element(indices.cbegin(), indices.cend());
    if (max_index >= inputs.size()) return std::nullopt;
    std::vector<double> values;
    values.reserve(N);
  ...
  /// @brief Set the function pointer of this node.
  /// @param inner_function The inner function for this node to use. Will be wrapped in a
  /// NodeFunction.
...
{
  auto vals = node.GetInputValues();
  return std::any_of(vals.cbegin(), vals.cend(), [](const double val) { return val == 0.; }) ? 0.
                                                                                             : 1.;
}
...
double AnyEq(const GraphNode &node, const cse491::AgentBase &)
{
  ...
  if (vals.size() == 0) return node.GetDefaultOutput();
  for (size_t i = 1; i < vals.size(); ++i) {
    if (vals.at(0) == vals.at(i)) return 1.;
  }
  return 0.;
}
...
double Not(const GraphNode &node, const cse491::AgentBase &)
{
  ...
  if (!vals.has_value()) return node.GetDefaultOutput();
  return (*vals)[0] == 0. ? 1. : 0.;
}
...
double Reciprocal(const GraphNode &node, const cse491::AgentBase &)
{
  ...
  return std::transform_reduce(
      PAR vals.cbegin(), vals.cend(), 0., std::plus{},
...
/// @brief Returns 1 if all inputs are in ascending, 0 otherwise. If only one input, then defaults
/// to 1.
/// @param node The node to get the inputs from.
/// @return The function result as a double.
...
{
  std::vector<double> vals = node.GetInputValues();
  if (vals.empty()) return node.GetDefaultOutput();
...
{
  //
  // The outputs of the first two connections are the x and y coordinates of the goal position. It'd
  // probably be rare for agents to randomly use it in a useful way. Most of the time when it IS
  // used, there is no input connections and thus the default output is used, so it isn't REALLY
  // being used. Other times when it does have input connections, the agent has a lower fitness, so
  // it probably wasn't making good use of it.
  //
  // Decided to make an easier way A* can be used by agents by giving the A* distance from the
  // agent's start position as an input. This can still be used in the off chance it is useful.
  auto vals = node.GetInputValues<2>(std::array<size_t, 2>{0, 1});
  if (!vals.has_value()) return node.GetDefaultOutput();
  auto vals2 = *vals;
...
static const std::vector<InnerFunction> SENSOR_FUNCTION_SET{
    WallDistanceUp, WallDistanceDown, WallDistanceLeft, WallDistanceRight, AStarDistance};
