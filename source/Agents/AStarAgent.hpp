...
public:
   ...
   */
  void SetRecalculate(const int recalculate) { recalculate_after_x_turns = recalculate; }
  ...
  void RecalculatePath()
  {
    ...
    if (!path.empty()) {
      path.pop_back();
    }  // Remove the val that we are currently at
    current_move_num = 0;
  }
                      ...
                      const cse491::agent_map_t & /* agent_map*/) override
  {
    ...
    if (!path.empty()) {
      auto pos = path.back();
      path.pop_back();
      if (pos == position.Above()) return action_map["up"];
      if (pos == position.Below()) return action_map["down"];
      if (pos == position.ToLeft()) return action_map["left"];
      if (pos == position.ToRight()) return action_map["right"];
    }
