 ...
 */
MainInterface::MainInterface(size_t id, const std::string &name)
    : InterfaceBase(id, name), mWindow(sf::VideoMode({1000, 800}), "Maze Window")
{
 ...
 */
std::vector<std::string> MainInterface::CreateVectorMaze(const WorldGrid &grid,
                                                         ...
                                                         const agent_map_t &agent_map)
{
  ...
  for (const auto &[id, item_ptr] : item_map) {
    ...
    if (pos.CellX() >= 0 && pos.CellY() >= 0 && pos.CellX() < grid.GetWidth() &&
        pos.CellY() < grid.GetHeight() && !item_ptr->IsOwned()) {
                             ...
                             const item_map_t &item_map, const agent_map_t &agent_map)
{
  ...
  [[maybe_unused]] bool success = renderTexture.create(
      {static_cast<unsigned int>(drawSpaceWidth), static_cast<unsigned int>(drawSpaceHeight)});
 ...
 */
std::vector<std::string> MainInterface::LargeDisplayGrid(
    const std::vector<std::string> &symbol_grid)
{
  // Determine the top-left corner of the 9x23 window
  int topLeftX = std::max(
      0, std::min(mPlayerPosition.x - COL / 2, static_cast<int>(symbol_grid[0].size()) - COL));
  int topLeftY = std::max(
      0, std::min(mPlayerPosition.y - ROW / 2, static_cast<int>(symbol_grid.size()) - ROW));
...
/**
 * @brief Calculates the total drawing space based on the grid dimensions and cell size and also the
 * center position of the drawing space.
 *
 * @param grid The WorldGrid representing the maze.
 ...
 * @param drawCenterX Reference to the variable to store the x-coordinate of the center of the
 * drawing space.
 * @param drawCenterY Reference to the variable to store the y-coordinate of the center of the
 * drawing space.
 */
void MainInterface::CalculateDrawSpace(const WorldGrid &grid, float cellSize, float &drawSpaceWidth,
                                       float &drawSpaceHeight, float &drawCenterX,
                                       float &drawCenterY)
{
  if (mGridSizeLarge) {
 ...
 * @param agent_map    The map of ids to agents in the maze.
 */
size_t MainInterface::SelectAction(const WorldGrid &grid, const type_options_t &type_options,
                                   const item_map_t &item_map, const agent_map_t &agent_map)
{
  ...
  if (mTextBox->IsSelected()) {
    ...
    switch (event.key.code) {
      ...
      case sf::Keyboard::T:
        ...
        if (GetName() == "Interface")
          action_id = GetActionID("drop");
 ...
 */
void MainInterface::MouseClickEvent(const sf::Event &event, const size_t /*entity_id*/,
                                    const item_map_t &item_map)
{
 ...
 * @param renderTexture The Texture for the whole grid
 */
void MainInterface::DrawWall(sf::RenderTexture &renderTexture, sf::RectangleShape &cellRect,
 ...
 * @param waitTime
 */
void MainInterface::setMInputWaitTime(double waitTime) { MainInterface::mInputWaitTime = waitTime; }
