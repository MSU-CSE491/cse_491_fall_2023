...
public:
  ...
  ServerInterface(size_t id, const std::string& name)
      : cse491::InterfaceBase(id, name), NetworkingInterface(id, name)
  {
  }
  ...
  static Packet gridToPacket(const cse491::WorldGrid& grid,
                             ...
                             const cse491::item_map_t& itemMap, const cse491::agent_map_t& agentMap)
  {
    ...
    for (const auto& [id, entityPtr] : itemMap) {
      ...
      if (!std::isnan(pos.GetX()) && !std::isnan(pos.GetY()) && pos.CellX() < grid.GetWidth() &&
          pos.CellY() < grid.GetHeight()) {
   ...
   */
  size_t SelectAction(const cse491::WorldGrid& grid, const cse491::type_options_t& typeOptions,
                      ...
                      const cse491::agent_map_t& agentMap) override
  {
    ...
    std::cout << "Sending action map to " << m_ip.value().toString() << " on port " << m_port
              << std::endl;
