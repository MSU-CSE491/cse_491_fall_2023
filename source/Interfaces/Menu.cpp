...
void Menu::Initialize(sf::Font &font, sf::Vector2f size)
{
  ...
  mMenuBar.emplace_back(
      std::make_unique<Button>("Menu", MENU_BUTTON_SIZE, backgroundcolor, textcolor, font));
  mMenuBar[0]->SetPosition({0, 0});
  mMenuBar.emplace_back(
      std::make_unique<Button>("Inventory", MENU_BUTTON_SIZE, backgroundcolor, textcolor, font));
  mMenuBar[1]->SetPosition({200, 0});
  mMenuBar.emplace_back(
      std::make_unique<Button>("Exit", MENU_BUTTON_SIZE, backgroundcolor, textcolor, font));
  mMenuBar[2]->SetPosition({400, 0});
  mMenuBar.emplace_back(
      std::make_unique<Button>("Normal", MENU_BUTTON_SIZE, backgroundcolor, textcolor, font));
  mMenuBar[3]->SetPosition({600, 0});
  mMenuBar.emplace_back(
      std::make_unique<Button>("Enlarge", MENU_BUTTON_SIZE, backgroundcolor, textcolor, font));
