 ...
 */
void Inventory::ConstructInventory(sf::Font &font,
                                   const std::vector<std::string> &interfaceAgentInventory)
{
  ...
  std::copy(interfaceAgentInventory.begin(), interfaceAgentInventory.end(), mItems.begin());
  ...
  for (int i = 0; i < mRow; i++) {
    ...
    for (int j = 0; j < mCol; j++) {
      v1.push_back(std::make_unique<Button>(
          "", sf::Vector2f{(mWorldSize.x) / mCol, (mWorldSize.y / 2 - 50) / mRow}, sf::Color::Black,
          sf::Color::White, font));
...
>>>>>>> 853997a7b7cbc8f12045130a882490c5524d8f0b
    }
