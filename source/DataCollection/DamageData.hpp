 ...
 * @brief A data collector for damage-related data.
 * @note Status: ALPHA
 **/
...
/**
 * @brief Represents damage related data between an agent and other entities (agents, items, grids,
 * etc)
 */
class DamageData {
private:
...
public:
   ...
   */
  DamageData(std::shared_ptr<cse491::AgentBase> agnt, std::shared_ptr<cse491::Entity> src, int amt)
      : agent(std::move(agnt)), source(std::move(src)), amount(amt)
  {
  }
