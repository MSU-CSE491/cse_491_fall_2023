...
/**
 * @brief Represents data collected for an agent, including their name, actions, positions, and
 * associated IDs.
 */
class AgentData {
private:
...
public:
   ...
   */
  void StorePositions(cse491::GridPosition pos) { position.push_back(pos); }

  /**
   * @brief Stores an action for the agent.
   * @param action The action to be stored.
   */
  void StoreAction(std::unordered_map<std::string, size_t> action) { actions.push_back(action); }

  /**
   * @brief Stores an agent ID associated with an action.
   * @param id The agent ID to be stored.
   */
  void StoreAgentId(int id) { action_ids.push_back(id); }
  ...
  int GetActionSize() const { return actions.size(); }
   ...
   */
  int GetPositionSize() const { return position.size(); }

  /**
   * @brief Gets the agent IDs associated with the stored actions.
   * @return The vector of agent IDs.
   */
  auto GetAgentIds() const { return action_ids; }

  std::string GetName() const { return name; }

  std::vector<cse491::GridPosition> GetPositions() const { return position; }
};
}  // namespace DataCollection