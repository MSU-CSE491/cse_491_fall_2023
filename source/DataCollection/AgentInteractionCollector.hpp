...
private:
  std::unordered_map<std::string, int>
      interactionData;  /// Data storage map of agent name to interactions.
public:
  /**
  ...
  /**
   * Get the amount of unique agents that occurred
   * @return int amount of agent occurrences
   */
  size_t GetUniqueInteractions() { return interactionData.size(); }

  /**
   * Increment occurrence amount for a certain agent.
